...
function createAICodesMarkup(items = [], adminMode = false) {
    if (!items.length) {
        return `<div class="empty-state">Nenhum código com IA registrado ainda.</div>`;
    }
            ...
            ${items.map(item => `
                <article class="author-code-card" data-code-id="${item.id}">
                    <div class="author-code-header">
                        ...
                        <span class="author-code-badge">IA</span>
                    </div>
...
async function loadCodes() {
    try {
        return normalizeCodes(await getCodes("AI_ASSISTED"));
    } catch (error) {
        console.warn("Não foi possível carregar códigos com IA.", error);
        return [];
    }
}

export async function AICodesPage() {
    const adminMode = localStorage.getItem("erickos-admin-mode") === "true";
    const aiCodes = await loadCodes();
        ...
        <div class="page-content">
            <div class="content-container">
                <section class="projects-page-hero">
                    <div>
                        <span class="consistency-page-badge"><span class="hero-badge-icon streak-icon"><i data-lucide="brain-circuit"></i></span>Códigos com IA</span>
                        <h1>Guarde prompts e trechos gerados por IA</h1>
                        <p>Organize prompts, instruções e ideias para visualizar o raciocínio por trás de cada uso da IA.</p>
                    </div>
                </section>
                ...
                <section class="projects-page-surface">
                    <input type="search" id="ai-code-search" placeholder="Buscar código com IA..." aria-label="Buscar códigos com IA" class="projects-search-input">
                    ${createAICodesMarkup(aiCodes, adminMode)}
                </section>
            </div>
        </div>
    `;
...
async function refreshAICodesPage() {
    const container = document.getElementById("page");
    if (!container) {
        return;
    }

    container.innerHTML = await AICodesPage();
    initAICodesPage();
}
...
export function initAICodesPage() {
    const container = document.getElementById("page");
    if (!container) {
        return;
    }

    if (container.dataset.aiCodeListenersAttached === "true") {
        return;
    }

    container.dataset.aiCodeListenersAttached = "true";
    ...
    const searchInput = document.getElementById("ai-code-search");
    if (searchInput) {
